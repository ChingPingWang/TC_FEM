...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    #                        name="mrcnn_mask")(x)
    # Conv layers
    x = KL.TimeDistributed(KL.Conv2DTranspose(128, (2, 2), strides=2, activation="relu"),name="mrcnn_mask_deconv_0")(x)
    x = KL.TimeDistributed(BatchNorm(),name='mrcnn_mask_deconv_bn0_0')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    s0 = x #up 2x*2x

    x = KL.TimeDistributed(KL.Conv2D(128, (3, 3),strides=(2,2) ,padding="same"),name="mrcnn_mask_conv1_1")(x)
    x = KL.TimeDistributed(BatchNorm(),name='mrcnn_mask_bn1_1')(x, training=train_bn)
    x = KL.Activation('relu')(x)
    s1 =x  # x*x

    x = KL.TimeDistributed(KL.Conv2D(128, (3, 3),strides=(2,2) ,padding="same"),name="mrcnn_mask_conv2")(x)
    ...
    x = KL.Activation('relu')(x)
    x = concatenate([x,s2],axis=-1) # x/2*x/2

    x = KL.TimeDistributed(KL.Conv2DTranspose(128, (2, 2), strides=2, activation="relu"),name="mrcnn_mask_deconv_1")(x)
    x = concatenate([x,s1],axis=-1) #up x*x
    
    x = KL.TimeDistributed(KL.Conv2DTranspose(128, (2, 2), strides=2, activation="relu"),name="mrcnn_mask_deconv")(x)
    x = concatenate([x,s0],axis=-1) #2x*2x
...
def expend_as(tensor, rep):
     return KL.Lambda(lambda x, repnum: K.repeat_elements(x, repnum, axis=3),
...
def attention_block(x, gating, inter_shape, name):
    ...
    theta_x = KL.Conv2D(inter_shape, (2, 2), strides=(2, 2), padding='same')(x)  
    shape_theta_x = K.int_shape(theta_x)

    phi_g = KL.Conv2D(inter_shape, (1, 1), padding='same')(gating)
    upsample_g = KL.Conv2DTranspose(inter_shape, (3, 3),
                                 strides=(1,1),
                                 padding='same')(phi_g)  

    concat_xg = KL.add([upsample_g, theta_x])
    act_xg = KL.Activation('relu')(concat_xg)
    psi = KL.Conv2D(1, (1, 1), padding='same')(act_xg)
    sigmoid_xg = KL.Activation('sigmoid')(psi)
    shape_sigmoid = K.int_shape(sigmoid_xg)
    upsample_psi = KL.UpSampling2D(size=(2,2),name=name+'_weight')(sigmoid_xg)  

    upsample_psi = expend_as(upsample_psi, shape_x[3])


    y = KL.multiply([upsample_psi, x])
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        P4_u = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c4p4')(C4)
        P4 = attention_block(P4_u, P5, config.TOP_DOWN_PYRAMID_SIZE, 'P4')
        
        P3_u = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c3p3')(C3)
        P3 = attention_block(P3_u, P4, config.TOP_DOWN_PYRAMID_SIZE, 'P3')

        P2_u = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c2p2')(C2)
        P2 = attention_block(P2_u, P3, config.TOP_DOWN_PYRAMID_SIZE, 'P2')
