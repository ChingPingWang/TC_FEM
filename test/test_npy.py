...
def overlap_preposes(input_):
    #
    result = input_[0]
    print('--------------------First_result:',result[0][2])
    size   = input_[1]
    th = 0.5
    # pdb.set_trace()
    print('--------------------size',size)
    result = np.array(result)
    print('--------------------Second_result:',result[0][2])
    ...
    for i in range((image.shape[0]//overlap[0]) + 1):
        for j in range((image.shape[1]//overlap[1]) + 1):
            ...
            for num, c in enumerate(class_ids):
                ...
                # pdb.set_trace()
                # if mask.max() != 0:
                # #if mask.max() != 0 and scores[num]>=0.73:
                #     #print(c)
                #     roi_new = [x0, x1, y0, y1]
                #     #
                ...
                #         print('ROI:',roi_new)
                roi_new = [x0, x1, y0, y1]
                roi_new = [int(x) for x in roi_new]
                if mask.max()>0 and x0<x1 and y0<y1:
                    mask_array = mask[int(rois[num,:][0]):int(rois[num,:][2]),int(rois[num,:][1]):int(rois[num,:][3])]
                    if mask_array.max()>0 :
                        masks_new.append([c, roi_new, mask_array])
                        # pdb.set_trace()
                        # print('ROI',roi_new,'MaxMask',mask.shape)
                        # print(masks_new)
...
file_path = '/mnt/disk6/nancy/paip_2023/data/PAIP2023_Test/'
#file_path = '/mnt/disk6/nancy/ki_paper/test_data/big/image/'
save_path = '/mnt/disk6/nancy/paip_2023/result/test_inside/mrcnn/test_152_luo/'
model = model_cell_load('/mnt/disk6/nancy/paip_2023/logs/mask_rcnn_nucleus_0152.h5')
